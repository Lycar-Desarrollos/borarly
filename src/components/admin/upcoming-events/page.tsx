...
export default function AdminUpcomingEventsPage() {
  ...
  const handleCheckboxChange = (checked: boolean) => {
      setCurrentEvent(prev => ({...prev, isActive: checked}));
  };
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                {currentEvent.imageUrl && <div className="relative mt-2 w-[200px] h-[100px] bg-white border rounded"><NextImage src={currentEvent.imageUrl} alt="Vista previa" layout="fill" objectFit="contain" className="p-1"/></div>}
              </div>
              <div>
                ...
                {currentEvent.brandLogoUrl && <div className="relative mt-2 w-[100px] h-[50px] bg-white border rounded"><NextImage src={currentEvent.brandLogoUrl} alt="Vista previa de logo" layout="fill" objectFit="contain" className="p-1"/></div>}
              </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
              ...
              <div>
               ...
               <div className="flex items-center space-x-2">
                <Checkbox id="isActive" name="isActive" checked={!!currentEvent.isActive} onCheckedChange={(checked) => handleCheckboxChange(Boolean(checked))} />
              ...
              <TableBody>
                {events.length > 0 ? events.map((event) => (
                  <TableRow key={event.id}>
                    <TableCell>
                      <div className="w-24 h-12 relative border rounded overflow-hidden bg-white">
                        <NextImage src={event.imageUrl} alt={event.title} layout="fill" objectFit="contain" className="p-1"/>
                      </div>
                    </TableCell>
