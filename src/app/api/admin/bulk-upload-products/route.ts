...
interface ProcessedRowResult {
  ...
  data?: Omit<ProductType, 'createdAt' | 'updatedAt'>; 
  error?: string;
        ...
        complete: async (parseResult) => {
          ...
          if (parseResult.errors.length > 0) {
            ...
            resolve(NextResponse.json({ 
              ...
              details: parseResult.errors.map(e => `Fila CSV ${e.row + 2}: ${e.message} (${e.code})`).join('; '),
              ...
              failedRowsDetails: parseResult.errors.map(e => ({
                csvRowNumber: e.row + 2,
                error: `${e.message} (${e.code})`,
                sku: parseResult.data[e.row]?.sku || "N/A",
                name: parseResult.data[e.row]?.name || "N/A",
              }))
            }, { status: 400 }));
            return;
          }
